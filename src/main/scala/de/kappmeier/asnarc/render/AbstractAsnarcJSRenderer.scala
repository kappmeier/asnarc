...
class AbstractAsnarcJSRenderer(canvas: html.Canvas, loc: AsnarcLocalization) {
  ...
  /**
   * Fills the square block of an [[Element]] of the Asnarc game. For a block standing alone only the interior is
   * filled, but when a block is connected with the neighbor block in a specific direction, the border in this direction
   * is also filled.
   *
   * Two blocks are not considered connected when they are actually neighbors. Instead, the `connects` property of the
   * [[Element]] must be set.
   *
